...
function readAllData(ss) {
  var keys = ['jobs','clients','tasks','timeEntries','costItems',
              'designFiles','users','sections','approvals','driveFiles','leads'];
  var result = {};
...
function saveLead(ss, lead) {
  try {
    ...
    // Append new lead row (for spreadsheet visibility)
    sheet.appendRow([
      lead.id || ('lead_' + Date.now()),
      lead.name, lead.firstname||'', lead.surname||'', lead.company || '', lead.email,
    ...
    // Also update the leads JSON blob so MIS reads it on next load
    var allLeads = [];
    try {
      var existing = sheet.getRange(2, 1).getValue();
      // Check if cell 2,1 has JSON array (MIS format) vs row data
      if (existing && existing.toString().trim().startsWith('[')) {
        allLeads = JSON.parse(existing);
      }
    } catch(e) { allLeads = []; }
    // Read all row-based leads and convert to JSON blob
    var allData = ss.getSheetByName('leads');
    var rows = allData ? allData.getDataRange().getValues() : [];
    // Row 1 is header, row 2 onwards are leads
    // But the MIS stores as JSON in cell(2,1) — we need to use writeAllData pattern
    // Simplest: read existing leads JSON from main data, add new lead, write back
    var mainKeys = ['jobs','clients','tasks','timeEntries','costItems',
                    'designFiles','users','sections','approvals','driveFiles'];
    var leadsSheet = getOrCreateSheet(ss, 'leads');
    var leadsRaw = leadsSheet.getRange(2, 1).getValue();
    var leadsArr = [];
    try { leadsArr = leadsRaw ? JSON.parse(leadsRaw) : []; } catch(e) { leadsArr = []; }
    if (!Array.isArray(leadsArr)) leadsArr = [];
    // Add new lead if not already there
    var exists = leadsArr.some(function(l){ return l.id === lead.id; });
    if (!exists) {
      leadsArr.push(lead);
      leadsSheet.getRange(2, 1).setValue(JSON.stringify(leadsArr));
    }
    ...
    try {
      MailApp.sendEmail({
        ...
        htmlBody: '<div style="font-family:sans-serif;max-width:520px">'
          ...
          + '<table style="width:100%;border-collapse:collapse">'
          + '<tr><td style="padding:6px 0;color:#666;font-size:13px;width:120px">First Name</td><td style="padding:6px 0;font-weight:700">' + (lead.firstname||lead.name) + '</td></tr>'
          + '<tr><td style="padding:6px 0;color:#666;font-size:13px">Surname</td><td style="padding:6px 0;font-weight:700">' + (lead.surname||'') + '</td></tr>'
